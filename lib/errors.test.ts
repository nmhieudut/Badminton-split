...
describe('ConfigError', () => {
  it('không để lộ chi tiết cấu hình ra thông báo người dùng thấy', () => {
    const error = new ConfigError('Thiếu biến môi trường DATABASE_URL');

    expect(error.message).not.toContain('DATABASE_URL');
    expect(error.message).not.toMatch(/env|\.env|Vercel|Supabase|postgres/i);
  });

  it('đưa ra một câu chung, đọc được, không đổ lỗi cho người dùng', () => {
    const error = new ConfigError('bất kỳ chi tiết nội bộ nào');
    expect(error.message).toBe('Hệ thống đang trục trặc. Vui lòng thử lại sau ít phút.');
  });
